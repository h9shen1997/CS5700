...
public class ProtocolResponse implements IProtocol {
  ...
  public static ProtocolResponse convertFromByteList(List<Byte> byteList) {
    short numOfAnswer = (short) (((byteList.get(0) & 0xff) << BYTE_SHIFT) | (byteList.get(1)
        & 0xff));
  ...
  public byte[] convertToByteArr() {
    ...
    byteArr.add((byte) ((numOfAnswers >> BYTE_SHIFT) & 0xff));
    byteArr.add((byte) (numOfAnswers & 0xff));
    for (int i = 0; i < numOfAnswers; i++) {
      byteArr.add((byte) ((lengthArr[i] >> BYTE_SHIFT) & 0xff));
